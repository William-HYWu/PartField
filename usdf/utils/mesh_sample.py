...
def create_mesh(
    ...
    # Filter samples to only include points with y > 0.4
    y_filter = samples[:, 1] > 0.4
    samples_filtered = samples[y_filter]
    filtered_indices = torch.where(y_filter)[0]
    
    num_samples = samples_filtered.shape[0]
    print(f"Filtered to {num_samples}/{N**3} samples with y > 0.4")

    samples_filtered.requires_grad = False

    head = 0

    while head < num_samples:
        sample_subset = samples_filtered[head : min(head + max_batch, num_samples), 0:3].cuda()

        samples_filtered[head : min(head + max_batch, num_samples), 3] = (
    ...
    # Reconstruct full grid with filtered SDF values
    # Points not evaluated (y <= 0.4) will have SDF = 1.0 (outside)
    samples[:, 3] = 1.0  # default to outside
    samples[filtered_indices, 3] = samples_filtered[:, 3]
    ...
    # Save voxel grid (sdf values and metadata) for debugging/visualization
    try:
        # ensure we have a numpy array for saving
        sdf_to_save = sdf_values_np if 'sdf_values_np' in locals() else (
            sdf_values.numpy() if isinstance(sdf_values, torch.Tensor) else np.array(sdf_values)
        )
        save_voxelgrid(sdf_to_save, voxel_origin, voxel_size, ply_filename + ".npz")
    except Exception as e:
        print(f"Warning: failed to save voxel grid: {e}")
...
def save_voxelgrid(sdf_np, voxel_origin, voxel_size, out_npz_path):
    """Save voxel grid SDF values and metadata to a compressed .npz file.

    Args:
        sdf_np (np.ndarray): grid shaped (N, N, N) of SDF values
        voxel_origin (list or tuple): origin coordinates [x,y,z]
        voxel_size (float): size of a voxel
        out_npz_path (str): output path for .npz
    """
    # Convert to numpy array if needed
    sdf_arr = np.array(sdf_np, copy=False)

    # Create sign mask: 1 for inside (sdf <= 0), 0 for outside
    sign_mask = (sdf_arr <= 0).astype(np.uint8)

    origin_arr = np.array(voxel_origin, dtype=np.float32)
    
    print("Saving voxel grid to:", out_npz_path)
    # Save compressed NPZ
    np.savez_compressed(
        out_npz_path,
        sdf=sdf_arr,
